// ============================================================
//  VeriChain AI — Enterprise Dashboard
//  Backend: https://verichain-backend.onrender.com
// ============================================================
...
const MOCK_DATA = [
  {
    ...
    aiDelayRisk: 'HIGH',
    aiEstimatedDelayHours: 12,
    aiConfidenceScore: 0.94,
    aiRecommendedAction: 'REROUTE',
  ...
  },
  {
    ...
    aiDelayRisk: 'LOW',
    aiEstimatedDelayHours: 0,
    aiConfidenceScore: 0.91,
    aiRecommendedAction: 'PROCEED',
...
async function fetchShipments() {
  ...
  try {
    ...
    if (!shipments || shipments.length === 0) {
      renderShipments(MOCK_DATA, grid, true);
      return;
    }

    renderShipments(shipments, grid, false);
    updateStats(shipments);

  } catch (err) {
    console.warn('Live backend unreachable — loading demo data:', err.message);
    loader.style.display = 'none';
...
// ── Render Cards ──────────────────────────────────────────────
function renderShipments(shipments, container, isMock) {
  container.innerHTML = '';

  shipments.forEach(shipment => {
    const statusClass = shipment.currentStatus.toLowerCase();
    const risk        = (shipment.aiDelayRisk || 'LOW').toLowerCase();
    const riskColor   = risk === 'high' ? 'high' : risk === 'medium' ? 'medium' : 'low';
    ...
    const onChainStatus = shipment.loggedOnChain
      ...
      : '<span style="color:#94a3b8; font-size:0.75rem;">Pending on-chain verification</span>';
    ...
    card.innerHTML = `
      ...
      <div class="ai-blockchain-panel">
        ...
        <div class="ai-prediction" style="border-left-color: ${borderColor}">
          <h3>
            <i data-lucide="brain" class="icon-small" style="color:${borderColor}"></i>
            AI Delay Prediction
          </h3>
          <div class="ai-value ${riskColor}">
            ${risk.toUpperCase()} RISK
            ${shipment.aiEstimatedDelayHours > 0 ? `— ${shipment.aiEstimatedDelayHours}h delay` : ''}
          </div>
          <div class="ai-meta">
        ...
        <div class="blockchain-ledger">
          <h3>
            <i data-lucide="shield-check" class="icon-small" style="color:#10b981"></i>
            Blockchain Ledger
          </h3>
          ${onChainStatus}
          ${txHash}
...
// ── Easter Egg (type "hire" anywhere on page) ─────────────────
function setupEasterEgg() {
  let keySequence = [];
  const secretCode = 'hire';
    ...
    if (keySequence.join('') === secretCode) {
      document.body.innerHTML = `
        <div class="terminal-screen">
          ...
          <p>&gt; Stack: Java · Spring Boot · Python · Web3j · React</p>
          <p>&gt; Architecture: Microservices · JWT · BCrypt · Blockchain</p>
          <p>&gt; Deployed: Render (backend) + Netlify (frontend)</p>
